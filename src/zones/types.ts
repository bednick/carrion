 ...
 *              лорно — бестелесные духи (`buried_shade`, `wailer`), см. `docs/factions.md`.
 * - `block`  — шанс 0..1 полностью погасить входящий удар (канал `block_chance`, то же короткое
 *              замыкание, что и `dodge`, но следующей стадией резолюции). Тоже инструмент Нежити,
 *              лорно — костяные/трупные мобы, принимающие удар на кость или щит. У Мародёров ни того,
 *              ни другого нет: их защита детерминирована (`armor` + `thorns`). Технически можно задать
 *              оба — см. приоритет отображения в UI (`src/ui/mobMechanics`).
 *
 * Полное гашение по шансу — фракционная монополия Нежити и причина, по которой против неё работает
 * частое лёгкое оружие, а не тяжёлое: матожидание гашение режет одинаково при любом темпе, но редкий
 * взмах теряет от одного неудачного ролла целый цикл (плюс оверкил мелкой полосы через `phases`).
