...
function runFight(hero: CombatState['hero'], zoneCfg: ZoneConfig, fightIdx: number, totalFights: number, isBoss: boolean): FightOutcome {
  ...
  while (state.phase === 'fighting') {
    ...
    elapsed += TICK_MS;
    if (elapsed > STALEMATE_MS) return 'stalemate';
  }
  return state.phase === 'dead' ? 'dead' : 'win';
}
...
export function runExpedition(equipment: Partial<Record<SlotType, ItemInstance>>, zoneCfg: ZoneConfig): ExpeditionResult {
  const hero = CombatEngine.buildInitialHero(equipment);
  ...
  for (let i = 0; i < fightPlan.length; i++) {
    ...
    if (isBoss) hpAtBossStart = hero.hp;
    const outcome = runFight(hero, zoneCfg, i, fightPlan.length, isBoss);
    if (outcome !== 'win') {
      return { outcome, fightsCompleted, hpAtBossStart, hpAtEnd: hero.hp };
    }
    if (!isBoss) fightsCompleted++;
  }
  return { outcome: 'win', fightsCompleted, hpAtBossStart, hpAtEnd: hero.hp };
}
