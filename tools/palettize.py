...
Аргументы:
  input                 входное изображение — любой формат, читаемый Pillow (обязателен,
                        кроме вызова с --list-palettes). Альфа-канал сохраняется.
  output                куда сохранить результат; формат по расширению (обязателен,
                        кроме --list-palettes).

  --colors N            целевое число цветов свободного k-means, диапазон 2-128
                        (по умолчанию 24). Взаимоисключающе с --palette.
  --palette P           работать по фиксированной палитре вместо свободного k-means.
                        P — одно из:
                          • имя встроенной палитры (список — через --list-palettes),
                            ищется как tools/palettes/<P>.png;
                          • путь к своей PNG-палитре (каждый уникальный цвет пикселя =
                            цвет палитры);
                          • 'auto' — число цветов подбирается автоматически методом
                            локтя (k до 64), затем строится палитра.
                        Взаимоисключающе с --colors.

  --no-downscale        не уменьшать изображение перед палетизацией. По умолчанию
                        уменьшение включено (даёт «крупный пиксель»).
  --scale D             делитель даунскейла/множитель обратного апскейла, диапазон 2-32
                        (по умолчанию 8). Если D не делит размеры нацело, итоговый
                        размер отличается от входного — печатается примечание.
                        Игнорируется при --no-downscale.
  --no-kcentroid        уменьшать простым билинейным ресайзом вместо K-Centroid
                        (тайл источника квантуется в 2 цвета, берётся самый частый).
                        По умолчанию используется K-Centroid — резче, но медленнее.
                        Действует только при включённом даунскейле.

  --dither {2,4,8}      порядок матрицы Байера: 2/4/8 = Bayer 2x2 / 4x4 / 8x8.
                        Сам по себе ничего не делает — нужен --dither-strength > 0.
                        Если задан --dither-strength, а --dither нет — берётся 8
                        (как в оригинале).
  --dither-strength S   сила упорядоченного (Bayer) дизеринга, диапазон 0-10
                        (по умолчанию 0 = дизеринг выключен, --dither не важен).
                        При S > 0 дополнительно применяется лёгкая гамма-коррекция
                        перед подбором цветов.

  --native-size         сохранить в уменьшенном (нативном) разрешении, без апскейла
                        ближайшим соседом обратно. Без эффекта при --no-downscale.
  --list-palettes       напечатать имена встроенных палитр и выйти (input/output
                        не требуются).
